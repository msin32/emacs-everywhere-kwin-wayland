...
(defun emacs-everywhere--app-info-linux-sway ()
  ...
  (let ((window-id (shell-command-to-string "swaymsg -t get_tree | jq '.. | select(.type?) | select(.focused==true) | .id'"))
	(app-name (shell-command-to-string "swaymsg -t get_tree | jq '.. | select(.type?) | select(.focused==true) | .app_id'"))
	(window-title (shell-command-to-string "swaymsg -t get_tree | jq '.. | select(.type?) | select(.focused==true) | .name'"))
	(window-geometry (list
			  (shell-command-to-string "swaymsg -t get_tree | jq '.. | select(.type?) | select(.focused==true) | .rect | .x'")
			  (shell-command-to-string "swaymsg -t get_tree | jq '.. | select(.type?) | select(.focused==true) | .rect | .y'")
			  '0 '0
			  (shell-command-to-string "swaymsg -t get_tree | jq '.. | select(.type?) | select(.focused==true) | .rect | .width'")
			  (shell-command-to-string "swaymsg -t get_tree | jq '.. | select(.type?) | select(.focused==true) | .rect | .height'"))))
  (list window-id app-name window-title window-geometry)))
